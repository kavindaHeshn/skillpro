...
try {
    ...
    // Handle Edit User (unchanged logic)
    if (isset($_POST['edit_user']) && $is_staff_or_above) {
        ...
        $role       = $is_admin ? $_POST['role'] : $_SESSION['role'];

        $allowed_roles_to_edit = ['student'];
        if ($is_admin) $allowed_roles_to_edit[] = 'instructor';
        ...
        if (in_array($current_role, $allowed_roles_to_edit) || $is_admin) {
            ...
            echo '<div class="error">You are not allowed to edit this user.</div>';
        }
        $check->close();
    }

    // Fetch users & counts (unchanged)
    ...
    $count_staff = $conn->query("SELECT COUNT(*) AS c FROM users WHERE role='staff' OR role='admin'")->fetch_assoc()['c']; // admin count staff table එකට
...
<div class="flex-container">
    ...
    <div class="main-content">
        ...
        <div class="container">
            ...
            <?php if (isset($_POST['edit_user'])) echo $success ?? $error ?? ''; ?>
                ...
                <table>
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()):
                            ...
                            $can_edit = $is_staff_or_above && in_array($row['role'], ['student', 'instructor']) || $is_admin;
                            $can_delete = $is_admin;
                        ?>
                            <tr>
